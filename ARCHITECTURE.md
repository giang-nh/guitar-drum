...
A small browser/Node-compatible `core-logic.js` now owns pure rules that are important enough to regression-test directly: row/section/phrase mapping, Health permission matrices and thresholds, section-transition eligibility, slow-tempo HOLD rejoin bounds, chord parsing/transposition/difficulty + capo recommendations, telemetry threshold replay and regression blockers. The browser player, Tone/Capo module and Follow module delegate those rules to this shared core so CI tests the same implementations used at runtime instead of copies.

Tests use Node's built-in `node:test`/`assert` only; there is no package install or test dependency. `tests/core.test.cjs` covers functional invariants including 4/8/12-bar phrase behavior, partial phrases, GREEN/YELLOW/RED permissions, 50 BPM rejoin timing, section-transition guards, slash-chord transposition, capo relation, replay pass/reject behavior and regression safety. `tests/contracts.test.cjs` compiles runtime JavaScript and checks integration contracts: shared-core load order/delegation, the exact three-song catalog and row timing sanity, tone-reference coverage, PWA precache contents, Playing/Developer wiring, phrase-vs-full-fill guards, Clean Mic voice coverage, workflow gating and debug-cache version consistency.

`.github/workflows/pages.yml` runs the test suite before Pages deployment and declares `deploy.needs: test`; a failure prevents upload/deploy. `.github/workflows/tests.yml` runs the same suite for pushes and pull requests as a standalone CI signal. The older source-extraction test runner was removed to avoid maintaining a second implementation-specific harness.


### Guided iPad Validation Session

Developer Mode now contains a guided, telemetry-only real-device validation workflow. The flow owns or reuses the current Debug Session and runs ten labeled scenarios: quiet room, drum-only speaker bleed, guitar-only onset/tempo acquisition, voice-only rejection, guitar+voice, full guitar+voice+drum mix, stop/resume HOLD→REJOIN, Verse→Chorus transition, soft→big dynamics, and free play. Each step emits `validation-step-start` / `validation-step-end` telemetry events and stores the step's telemetry-relative start/end times; no audio waveform or recording is stored.

Validation scoring lives in shared `core-logic.js`, not only in UI code. `validationWindowMetrics()` summarizes onset deltas, contamination class fractions, guitar/drum/voice evidence, Health distribution, tempo/bar lock time, HOLD/REJOIN observations, section changes, planner activity and soft/big dynamics. `validationStepStatus()` maps those metrics to `pass / review / fail` with conservative thresholds. `buildValidationReport()` aggregates the guided run into mic-separation status, median tempo/bar lock, false drum/voice onset counts, HOLD→REJOIN result, transition result and weighted RED-Health time.

A completed Validation Session can be pushed directly into the in-memory Regression suite, so later Auto-Tune candidates are replayed against the same real-device session. The `Analyze failures` action reuses the current telemetry/validation marks as Auto-Tune evidence. Exported debug JSON includes the current validation report and v34 cache/schema context; older JSON remains import-compatible through existing fallbacks.

The validation workflow is intentionally observational and device-driven. It does not synthesize fake microphone inputs or claim that CI proves acoustic performance. Automated tests cover the scoring/reporting rules and wiring; the iPad run supplies the real microphone/speaker evidence.
